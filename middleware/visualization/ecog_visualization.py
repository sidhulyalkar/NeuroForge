...
class SignalQualityAnalyzer:
    ...
    def assess_signal_quality(self, data, fs=1000, meta=None):
        ...
        for ch in range(n_channels):
            ...
            # Artifact detection
            quality_metrics["artifact_ratio"][ch] = self._detect_artifacts(
                channel_data, fs
            )
        ...
        quality_metrics["recommendations"] = self._generate_recommendations(
            quality_metrics
        )
...
class EnhancedECoGVisualizer:
    ...
    def __init__(self, meta_df, clean_data, sampling_rate=1000):
        ...
        if isinstance(meta_df, pd.DataFrame):
            ...
            self.z_mm = (
                self.meta.get("z_m", pd.Series(np.zeros(len(self.meta)))).values * 1000
            )
            ...
            st.warning(
                "No metadata DataFrame provided. Generating synthetic electrode positions."
            )
            ...
            try:
                ...
                st.error(
                    f"Unsupported metadata type: {type(meta_df)}. Expected pandas DataFrame."
                )
    ...
    def plot_quality_overview(self):
        ...
        fig.add_trace(
            go.Histogram(
                x=self.quality_metrics["snr"], name="SNR Distribution", nbinsx=20
            ),
            row=1,
            col=2,
        )
        ...
        fig.add_trace(
            go.Scatter(
                ...
                text=[
                    f"Ch{i+1}: {score:.2f}"
                    for i, score in enumerate(self.quality_metrics["channel_scores"])
                ],
    ...
    def plot_electrode_layout_advanced(self, color_by="quality", show_labels=True):
        ...
        for i in range(len(self.x_mm)):
            ...
            if self.quality_metrics is not None:
                ...
                text += (
                    f"Artifact Ratio: {self.quality_metrics['artifact_ratio'][i]:.3f}"
                )
            ...
            if len(bad_channels) > 0:
                fig.add_trace(
                    go.Scatter(
                        ...
                        mode="markers",
                        marker=dict(
                            size=25, color="red", symbol="x", line=dict(width=3)
                        ),
                        name="Poor Quality",
                        hoverinfo="skip",
                    )
    ...
    def plot_brain_surface_3d_enhanced(
        self, brain_radius=None, color_by="activity", show_brain=True
    ):
        ...
        if brain_radius is None:
            ...
            max_dist = np.max(
                np.sqrt((self.x_mm - center_x) ** 2 + (self.y_mm - center_y) ** 2)
            )
    ...
    def plot_spectral_analysis(self, freq_bands=None):
        ...
        for band_name, (low, high) in freq_bands.items():
            ...
            for ch in range(self.n_channels):
                freqs, psd = signal.welch(
                    self.clean[ch], self.fs, nperseg=min(self.n_samples // 4, 512)
                )
        ...
        for i, (band_name, power) in enumerate(band_powers.items()):
            ...
            power_norm = (power - np.min(power)) / (
                np.max(power) - np.min(power) + 1e-12
            )
...
def create_enhanced_ecog_section(meta, clean_data, sampling_rate=1000):
    ...
    st.write(
        f"Meta shape/length: {getattr(meta, 'shape', len(meta) if meta is not None else 'None')}"
    )
    ...
    try:
        ...
        with st.expander("📊 Signal Quality Assessment", expanded=True):
            if st.button("Analyze Signal Quality"):
                ...
                with col2:
                    st.metric(
                        "Average SNR", f"{np.mean(quality_metrics['snr']):.1f} dB"
                    )
                with col3:
                    st.metric(
                        "Artifact Level",
        ...
        with viz_tabs[0]:
            ...
            with col2:
                ...
                show_labels = st.checkbox(
                    "Show channel labels", value=True, key="enhanced_labels"
                )

            with col1:
                enhanced_fig = visualizer.plot_electrode_layout_advanced(
                    color_by=color_option, show_labels=show_labels
                )
        ...
        with viz_tabs[1]:
            ...
            with col2:
                ...
                show_brain = st.checkbox(
                    "Show brain surface", value=True, key="show_brain"
                )

            with col1:
                fig_3d_enhanced = visualizer.plot_brain_surface_3d_enhanced(
                    brain_radius=brain_radius, color_by=color_3d, show_brain=show_brain
                )
        ...
        with viz_tabs[2]:
            ...
            alpha_power = np.mean(
                [
                    np.sqrt(np.mean(clean_data[ch] ** 2))
        ...
        with viz_tabs[3]:
            ...
            with col1:
                st.metric(
                    "Max Correlation",
                    f"{np.max(correlation_matrix[correlation_matrix < 1]):.3f}",
                )
            with col2:
                st.metric(
                    "Mean Correlation",
                    f"{np.mean(correlation_matrix[correlation_matrix < 1]):.3f}",
                )
            with col3:
                st.metric(
                    "Network Density",
                    f"{np.mean(np.abs(correlation_matrix) > 0.3):.1%}",
                )
...
def create_basic_ecog_fallback(clean_data):
    ...
    fig = go.Figure(
        data=go.Heatmap(
            z=corr, colorscale="RdBu", zmid=0, colorbar=dict(title="Correlation")
        )
    )
