...
with tabs[2]:
    ...
    if st.button("Run Pipeline with Synthetic Channel Labels"):
        ...
        with st.spinner("Running raw pipeline…"):
            out0 = run_full_pipeline(
                "hardware_profiles/openbci_cyton.yaml", mode=mode, labels=None
            )
        arr, _ = out0["raw"]
        ...
        with st.spinner("Running pipeline with labels…"):
            result = run_full_pipeline(
                "hardware_profiles/openbci_cyton.yaml", mode=mode, labels=labels
            )
    ...
    if st.button("Run Pipeline"):
        ...
        if mode == "ECoG":
            ...
            try:
                ...
                from middleware.visualization.ecog_visualization import (
                    create_enhanced_ecog_section,
                )

                # Use enhanced visualization - CORRECT FUNCTION CALL
                st.subheader("🧠 Enhanced ECoG Array Visualization")
                visualizer = create_enhanced_ecog_section(
                    meta, clean, sampling_rate=1000
                )
                ...
                with analysis_tabs[0]:
                    ...
                    with col2:
                        ...
                        st.write(
                            f"Recording duration: {clean.shape[1]/1000:.1f} seconds"
                        )
                ...
                if (
                    meta is not None
                    and isinstance(meta, pd.DataFrame)
                    and "x_m" in meta.columns
                ):
                ...
                st.write(
                    f"Successfully processed {clean.shape[0]} channels with {clean.shape[1]} samples"
                )
            ...
            quality_df = pd.DataFrame(
                {
                    ...
                    "Peak-to-Peak": np.ptp(clean, axis=1),
                    "Quality Score": np.random.uniform(
                        0.6, 0.95, clean.shape[0]
                    ),  # Placeholder
                }
            )
...
with tabs[6]:
    ...
    if st.button("Start Real-Time"):
        ...
        def on_window(preds, ts):
            # store the mean prediction over channels (or whatever)
            st.session_state.logs.append((ts, preds.mean()))

        client.start_processing_pipeline(
            fs=250,
            preprocess_fn=lambda data, fs: preprocess(data, fs, hw_steps),
            feature_fn=lambda clean, fs: extract_features(clean, fs, feat_spec),
            decode_fn=lambda feats, spec: decode(feats, spec),
            decoding_spec={"model": "RandomForestClassifier"},
            window_size_s=1.0,
            step_size_s=0.5,
            callback=on_window,
        )
    ...
    if st.button("Stop Real-Time"):
        client.stop_processing_pipeline()
        st.warning("Real-Time Stopped")
